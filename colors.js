  ...
  methods: {
    ...
    filterColorName: function (e) {
      ...
      const colors = this.colors.filter((col) => {
        return (
          col.name.toLowerCase().indexOf(name) > -1 ||
          col.hex.indexOf(name) > -1
        );
      });

      this.colorList = colors.length
        ? colors
        : [
            {
              hex: "#f0c",
...
xhr.onload = (e) => {
  if (xhr.status === 200) {
    ...
    app.closestColors = new Closest(
      resp.colors.map((c) => [c.rgb.r, c.rgb.g, c.rgb.b]),
      true
    );
